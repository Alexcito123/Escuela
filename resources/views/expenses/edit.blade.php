...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Gasto</h1>
        </div>
            ...
            <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="concepto" value="{{ old('concepto', $expense->concepto) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" min="0.01" name="monto" value="{{ old('monto', $expense->monto) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha" value="{{ old('fecha', $expense->fecha->format('Y-m-d')) }}" required
                               class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        ...
                        <select name="metodo_pago" required
                            ...
                            @foreach ($metodos as $m)
                                <option value="{{ $m }}" {{ old('metodo_pago', $expense->metodo_pago) == $m ? 'selected' : '' }}>{{ $m }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="proveedor" value="{{ old('proveedor', $expense->proveedor) }}"
                           class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <textarea name="observaciones" rows="3"
                              class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('observaciones', $expense->observaciones) }}</textarea>
                </div>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white px-6 py-2 rounded-lg font-medium transition">Actualizar Gasto</button>
                </div>
            </form>
        </div>
    </div>
