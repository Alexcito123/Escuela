...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $payments->total() }} registros</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                <select name="concepto"
                    ...
                    @foreach ($conceptos as $c)
                        <option value="{{ $c }}" {{ request('concepto') == $c ? 'selected' : '' }}>{{ $c }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('payments.index') }}" class="bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 px-4 py-2 rounded-lg text-sm font-medium transition">Limpiar</a>
            </div>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($payments as $payment)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition">
                            <td class="px-6 py-4">
                                <a href="{{ route('payments.student', $payment->student) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400">
                                    {{ $payment->student->nombre_completo }}
                                </a>
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $payment->student->grade->name ?? '' }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('payments.destroy', $payment) }}" onsubmit="return confirm('¿Eliminar este pago?')" class="inline">
                                        @csrf @method('DELETE')
