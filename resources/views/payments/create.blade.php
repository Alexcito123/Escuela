...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Nuevo Pago</h1>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
            <form method="POST" action="{{ route('payments.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="student_id" required
                            ...
                            @foreach ($students as $student)
                                <option value="{{ $student->id }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>{{ $student->nombre_completo }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="concepto" required
                            ...
                            @foreach ($conceptos as $c)
                                <option value="{{ $c }}" {{ old('concepto') == $c ? 'selected' : '' }}>{{ $c }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="referencia" value="{{ old('referencia') }}" maxlength="255"
                               class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               placeholder="Folio, nº de transferencia...">
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium transition">Guardar Pago</button>
                </div>
            </form>
        </div>
    </div>
